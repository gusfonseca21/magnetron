...
def test_empty_dados_raises_error(empty_dados_legislatura):
    ...
    with pytest.raises(ValueError, match="Não foram encontrados dados sobre Legislatura"):
...
def test_missing_property_raises_error(missing_property):
    ...
    with pytest.raises(ValueError, match="A propriedade 'id' não existe dentro de Legislatura"):
...
def test_id_as_string_number_converts():
    ...
    data = {"dados": [{"id": "57", "dataInicio": "2023-02-01", "dataFim": "2027-01-31"}]}
    result = get_legislatura_data(data, "id")
    assert result == 57
    assert isinstance(result, int)